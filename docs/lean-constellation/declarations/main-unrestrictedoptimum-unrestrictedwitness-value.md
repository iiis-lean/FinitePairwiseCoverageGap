...
# `unrestrictedWitness_value`

The exact three-support witness has expected coverage four.

- Kind: `theorem`
- Node: `Main.UnrestrictedOptimum`
- Module: `FinitePairwiseCoverageGap.Main.UnrestrictedOptimum.Theorems.unrestrictedWitness_value`
- State: `proved`
...
Theorem `unrestrictedWitness_value` states the exact real-valued equality `expectedCoverage unrestrictedWitness = 4`.  It records that the three support sets of the accepted witness each cover all four features and that their weights yield expected coverage `4`.
...
Unfold `expectedCoverage`, `unrestrictedWitness`, `coverage`, and `elementFeatures`.  Evaluate the finite sum over all `Finset Element`: the witness vanishes except at `{0,2}`, `{1}`, and `{3,4}`.  For each of those three sets, compute the `biUnion` of the assigned feature sets and its cardinality; each is the full four-feature set, hence has coverage `4`.  The expected-coverage sum therefore becomes
`(3/10) * 4 + (7/20) * 4 + (7/20) * 4`.
Use finite big-operator normalization and exact rational real arithmetic to reduce this expression to `4`.  This establishes the exact accepted equality without asserting anything about sets outside the witness support.
...
- `Mathlib:Mathlib.Algebra.Ring.Defs.add_mul`
...
- `article/sections/02_counterexample.tex:37-39`
